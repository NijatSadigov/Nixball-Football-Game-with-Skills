...
export class Renderer {
  ...
  // Cosmetic shot effects (purchasable) — drawn via the shared paint routine
  // so the lobby skin previews look identical to the in-game burst.
  private drawShot(
    ...
    const t = (now - ef.t0) / shotDuration(ef.style);
    if (t > 1) return;
    paintShot(ctx, ef.style ?? 'classic', x, y, s, t, ef.seed ?? 0);
  }
}
